...
\newcommand{\panel}[2]{%
  \begin{beamercolorbox}[wd=\linewidth,sep=0.46cm,rounded=true]{panel}
...
\newcommand{\smallpanel}[2]{%
  \begin{beamercolorbox}[wd=\linewidth,sep=0.40cm,rounded=true]{smallpanel}
...
\newcommand{\resultcard}[3]{%
  \begin{beamercolorbox}[wd=\linewidth,sep=0.44cm,rounded=true]{takeaway}
...
\tikzset{
  ...
  arrow/.style={-{Latex[length=4mm]},very thick,draw=deepteal},
  softarrow/.style={-{Latex[length=4mm]},thick,draw=linegray}
}

\begin{document}
\begin{frame}[t]

\begin{beamercolorbox}[wd=\paperwidth,sep=0.98cm]{headline}
  \centering
  {\bfseries\fontsize{54}{62}\selectfont Influence Maximization When Communication Is the Bottleneck}\par
  \vspace{0.30cm}
  {\fontsize{30}{38}\selectfont From centralized submodular greedy to CELF acceleration and local candidate sharing}\par
  \vspace{0.28cm}
  {\fontsize{24}{31}\selectfont Yixin Chen \quad Bichi Zhang \quad Yaoqin Ye}\par
  \vspace{0.10cm}
  {\fontsize{19}{26}\selectfont CS240: Algorithm Design and Analysis, Spring 2026}
\end{beamercolorbox}

\vspace{0.58cm}
\begin{beamercolorbox}[wd=\textwidth,sep=0.50cm,rounded=true]{takeaway}
  \centering
...
\smallpanel{Benchmark Graphs}{
  \centering
  \includegraphics[height=11.1cm,keepaspectratio]{outputs/influence_maximization/figures/graphs/small_graph_structures.png}
...
\panel{Algorithmic Story}{
  \centering
  \begin{tikzpicture}[node distance=0.90cm and 0.95cm]
    \node[algo,text width=7.00cm] (simple) {Structure baselines\\Random, degree, PageRank};
    \node[algo,right=of simple,text width=7.10cm] (greedy) {Centralized greedy\\best MC marginal gain};
    \node[algo,right=of greedy,text width=7.00cm] (celf) {CELF\\lazy marginal refresh};
    \node[algo,below=1.55cm of greedy,text width=7.60cm] (dist) {GreeDI-style\\local candidates + coordinator};

    \draw[arrow] (simple) -- (greedy);
    \draw[arrow] (greedy) -- (celf);
    \draw[arrow] (greedy) -- (dist);

    \node[note,below=0.95cm of simple] (q1) {How strong are simple graph heuristics?};
    \node[note,below=0.95cm of dist] (q2) {How much quality survives limited communication?};
    \node[note,below=0.95cm of celf] (q3) {How many spread calls can submodularity save?};
...
\panel{Distributed Candidate Sharing}{
  \centering
  \begin{tikzpicture}[node distance=0.48cm and 0.76cm]
    ...
    \node[algo,right=1.10cm of p2,text width=6.50cm] (pool) {Union of top-\(q\) candidates};
    \node[algo,right=of pool,text width=6.10cm] (coord) {Coordinator greedy};
    \node[algo,right=of coord,text width=4.95cm] (seeds) {Final seeds};
...
\vspace{0.60cm}
\begin{beamercolorbox}[wd=\textwidth,sep=0.42cm,rounded=true]{smallpanel}
  \begin{columns}[T,totalwidth=\textwidth]
...
\vspace{0.26cm}
\begin{beamercolorbox}[wd=\textwidth,sep=0.28cm,rounded=true]{takeaway}
  \centering
