...
\textbf{Kempe, Kleinberg, and Tardos}~\cite{kempe2003} 系统提出了社交网络中的影响力最大化问题，证明了该问题在 IC 和 LT 模型下是 NP-hard 的，并利用子模最大化理论说明贪心算法可以达到 $(1-1/e)$ 的近似比。这是我们\textbf{重点复现的代表性论文}。
...
\textbf{Mirzasoleiman 等人}~\cite{mirzasoleiman2013} 提出了 GreeDI，这是一种针对基数约束下子模最大化问题的分布式协议。GreeDI 将基础集合划分到多台机器上，在每个分区上运行局部贪心算法，最后合并结果。它达到了常数因子的近似，并可直接应用于分布式影响力最大化。
...
张弼弛 & 算法 & 贪心、CELF/CELF++、TIM/IMM 算法实现与分析 \\
叶瑶勤 & 实验 & 数据集处理、实验评估、可视化与分布式扩展 \\
\bottomrule
\end{tabular}
